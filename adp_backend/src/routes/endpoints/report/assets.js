...
const packName = 'global.adp.endpoints.report.asset';

/**
* @swagger
* /report/assets/{format}:
*    post:
*      description: Generate report for Microservice
*      responses:
*        '200':
...
*                  assets:
*                    type: array
*                    example: [{ "_id": "61aa26a5a64ac0f2cd95af5e"}]
*    parameters:
*      - name: format
*        in: path
...
module.exports = (REQ, RES) => {
  ...
  const assetReports = new global.adp.quickReports.AssetReports(
    allowedAssets,
    format,
    fromDate,
    toDate,
  ...
  assetReports.generate().then((result) => {
    answer.setCode(200);
    ...
    const errorText = `Error in [ assetReports.generate ] in ${finalTimer}ms`;
