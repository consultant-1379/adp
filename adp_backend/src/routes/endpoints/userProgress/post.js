// ============================================================================================= //
/**
* [ global.adp.endpoints.userProgress.post ]
* Save a status of progress for a specific user
...
module.exports = async (REQ, RES) => {
  ...
  const packName = 'global.adp.endpoints.userProgress.post';
  ...
  global.adp.permission.getUserFromRequestObject(REQ)
    .then((USER) => {
      const { signum } = USER;
      global.adp.userProgress.save(signum, wid, REQ.rbac, alternativeMode)
        .then((RESULT) => {
          answer.setCode(200);
          answer.setData(RESULT);
          answer.setMessage('Progress successful saved!');
          const timer = (new Date()).getTime() - startTimer.getTime();
          answer.setCache(`Not from Cache - ${timer}ms`);
          answer.setTime(timer);
          res.statusCode = 200;
          res.end(answer.getAnswer());
        })
        .catch((ERROROBJ) => {
          adp.echoLog('Error calling [ adp.userProgress.save ]', { error: ERROROBJ }, 500, packName, true);
    ...
    .catch((ERROR) => {
      adp.echoLog('Error calling [ adp.permission.getUserFromRequestObject ]', { error: ERROR }, 500, packName, true);
      answer.setCode(500);
